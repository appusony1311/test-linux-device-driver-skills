...
//procedure-steps
/* step 1: giving the major and minor number that are unused , by knowing from cat /proc/devices ,as input to mkdev functions
   step 2: register_chrdev_region , will take theree arguments( dev_t , count, name)
...
/* API:functions used
 dev_t mkdev(int,int);
 int register_chrdev_region(dev_t, int count,char *name);
...
static int __init char_dev_init(void)
{
	...
	sample_drv.count=1;
	sample_drv.inuse=0;
	sample_drv.sample_dev = MKDEV(MAJORNO,MINORNO);
	if(register_chrdev_region(sample_drv.sample_dev,sample_drv.count,CHAR_DEV_NAME)) {
		pr_err("Error in alocationg the major and minor number for device\n");
		return -1;
	}
