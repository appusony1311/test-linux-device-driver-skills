...
static ssize_t char_dev_read(struct file *file, char *buf,  size_t lbuf,  loff_t *ppos)
{
	...
	pr_info("READER: trying for critical region lock \n");
	
	if(down_interruptible(&sample_drv.sample_lock))/*begin of critical region */
		/* something went wrong with wait */
                  return -ERESTARTSYS;
		
	pr_info("READER : acquired lock: executing critical code\n");
	...
	*ppos += nbytes;
	...
	up(&sample_drv.sample_lock); /*end of critical region */
...
static int __init char_dev_init(void)
{
	...
	sema_init(&sample_drv.sample_lock,1);
