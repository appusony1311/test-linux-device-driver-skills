...
static ssize_t sample_dev_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	char local_buf[]="sample driver example sysfs demo";
	return sprintf(buf,"%s",local_buf);

}
//static DEVICE_ATTR(dev,0666,sample_dev_show,NULL);
static struct device_attribute dev_attr_dev =
	__ATTR(sample_drv.sample_class, S_IRUGO, sample_dev_show, NULL);
...
static int __init char_dev_init(void)
{
	...
	memset(char_device_buf,'\0',MAX_LENGTH);
        ...
        sample_drv.sample_device=device_create (sample_drv.sample_class, NULL, sample_drv.sample_dev, NULL, "%s", "sample_cdrv");
	device_create_file(sample_drv.sample_device,&dev_attr_dev);
...
{
	 device_remove_file(sample_drv.sample_device,&dev_attr_dev);
